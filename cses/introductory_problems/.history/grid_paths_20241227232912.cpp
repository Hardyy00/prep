...
ll dfs(ll idx, ll r, ll c, ll n ,string &s, vector<vector<bool>> &visit){
	...
	if(s[idx] =='?'){
		fauto(i, dir){
			...
			res += dfs(idx+1, r + mp[i].F, c + mp[i].S,n,s, visit);
		}
	}else{
		res = dfs(idx+1, r + mp[s[idx]].F, c + mp[s[idx]].S , n, s , visit);
	}
